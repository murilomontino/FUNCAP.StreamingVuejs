/**
 * @typedef ArtisticDrawings
 * @type { Object }
 * @property { string } artisticDrawings.id - ID único do video
 * @property { string } artisticDrawings.publishedAt - Data de Publicação
 * @property { string } artisticDrawings.title - Titulo
 * @property { string } artisticDrawings.description - Descrição
 * @property { Thumbnail[] } artisticDrawings.thumbnails - Imagens dos Videos
 * @property { Videos[] } artisticDrawings.videos
 *
 *
 * @typedef Videos
 * @type {Object}
 * @property { string } video.description - Descrição
 * @property { string } video.image
 * @property { string } video.title
 * @property { string } video.id
 *
 * @typedef ConfigThumbnail
 * @type { Object }
 * @property { string } config.url
 * @property { number } config.width
 * @property { number } config.height
 *
 * @typedef Thumbnail
 * @type { Object }
 * @property { ConfigThumbnail } thumbnail.default
 * @property { ConfigThumbnail } thumbnail.medium
 * @property { ConfigThumbnail } thumbnail.high
 * @property { ConfigThumbnail } thumbnail.standard
 *
 *
 */

export default {
  /** @type {ArtisticDrawings[]} */
  artisticDrawings: []
}
